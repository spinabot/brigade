...
import { Tideline } from "../../api/tideline.js";
...
describe("memory-mcp-server dispatch", () => {
	...
	it("async backend errors normalize invalid ids and preserve notification silence", async () => {
		const tide = Tideline.open(dir, { hostPorts: { getBackend: () => ({
			ready: async () => { throw new Error("backend unavailable"); },
			readAll: () => [], writeAll: () => {}, appendEvent: () => {}, readEventsAsync: async () => [],
		}) } });
		const s = createMemoryMcpServer(tide, { origin: { kind: "owner" } });
		assert.ok(s.handleAsync);
		const malformed = { jsonrpc: "2.0", id: { invalid: true }, method: "tools/call" } as unknown as JsonRpcRequest;
		const response = await s.handleAsync(malformed);
		assert.equal(response?.id, null);
		assert.equal(response?.error?.code, -32603);
		assert.equal(await s.handleAsync(req(undefined, "tools/call")), null);
	});
