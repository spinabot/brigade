...
// The scanner is a pure shared utility; no runtime binding can disable it.
import { MemoryThreatError, scanForThreats } from "../../security/injection-patterns.js";
import type { FactStoreBackend, FactStoreHostPorts } from "../ports/host-ports.js";
export type { FactStoreBackend, FactStoreHostPorts } from "../ports/host-ports.js";
import { bm25Score, type ScoreBreakdown, tokenize } from "../retrieval/scoring.js";
import { evaluateWriteGate, isTrustedTarget, isUntrustedSource, WriteGateError } from "../governance/write-gate.js";
import { inverseLinkKind, type MemoryLink } from "../graph/links.js";
import { MemoryEventLog, type MemoryEvent } from "./event-log.js";
import { cosine, getDefaultEmbedder } from "../embeddings/embedder.js";
import { recallHybrid, recallHybridAsync } from "../retrieval/hybrid.js";
...
/** Instance-specific clock and optional persistence/logging integration. */
 ...
 * half-written store. Hosts may inject the legacy synchronous backend seam.
 */
export class FactStore {
	private readonly file: string;
	private readonly workspaceDir: string;
	private readonly hostPorts: FactStoreHostPorts;
	...
	constructor(workspaceDir: string, opts: FactStoreOptions = {}) {
		...
		// Match the legacy workspace derivation, including relative path normalization.
		this.workspaceDir = path.dirname(path.dirname(this.file));
		this.hostPorts = opts.hostPorts ?? {};
	...
	/** Whether this instance currently uses its built-in filesystem persistence. */
	get isFilesystemBacked(): boolean {
		return this.backend() === undefined;
	}

	/** Hosts with asynchronous hydration must become ready before synchronous reads. */
	async ready(): Promise<void> {
		await this.backend()?.ready?.();
	}

	/** Await fact persistence when the host writes behind; filesystem writes are synchronous. */
	async flush(): Promise<void> {
		await this.backend()?.flush?.();
	}

	private backend(): FactStoreBackend | undefined {
		return this.hostPorts.getBackend?.(this.workspaceDir);
	}
	...
	/** Emit additive provenance through this instance's backend or local event log. */
	private emit(event: MemoryEvent): void {
		const backend = this.backend();
		if (backend) {
			backend.appendEvent(event);
			return;
		}
		this.eventLog().append(event);
	}

	/** Synchronous history is filesystem-only; use readEventsAsync for other backends. */
	readEvents(): MemoryEvent[] {
		if (this.backend()) return [];
		return this.eventLog().readAll();
	}

	/** The append-only provenance history for either realization, ordered oldest-first. */
	async readEventsAsync(): Promise<MemoryEvent[]> {
		const backend = this.backend();
		if (backend) return backend.readEventsAsync();
		return this.eventLog().readAll();
	}
	...
	readAll(): MemoryRecord[] {
		const backend = this.backend();
		if (backend) return backend.readAll();
	...
	private writeAll(records: MemoryRecord[]): void {
		// Every mutator uses the same instance-bound persistence choke point.
		const backend = this.backend();
		if (backend) {
			backend.writeAll(records);
			return;
		}
		...
		if (this.lastReadHadUnparseable) {
			try {
				...
				this.hostPorts.logger?.warn("facts.jsonl had unparseable line(s); snapshotted before overwrite so nothing is silently dropped", { bak });
			} catch {
