 ...
 *   model) this lane uses content-only query terms plus the `minSim` floor, so
 *   question scaffolding cannot supply a recovery signal. Drop a LEARNED
...
import { effectiveScore } from "../lifecycle/decay.js";
import { cosine, type Embedder } from "../embeddings/embedder.js";
import type { MemoryRecord, MemorySourceType } from "../store/records.js";
import { bm25Score, tokenize } from "./scoring.js";

// Vector-lane floor, combined with content-only queries for the bundled
// bag-of-words embedders. A floor alone cannot distinguish question-scaffolding
// overlap from genuine morphology. Learned embedders may recommend their own.
...
/** Shared by sync recall and async pre-embedding shims, including graph recall. */
export function prepareRecallQuery(query: string, embedder: Embedder): string {
	return embedder.queryPreprocessing === "content-terms" ? tokenize(query).join(" ") : query;
}
...
export function recallHybrid(
	...
	const vectorQuery = prepareRecallQuery(query, embedder);
	// An all-stopword query has no recovery evidence. In particular, never embed
	// it as the bundled HRR's nonzero empty-text sentinel.
	const qv = vectorQuery.length > 0 ? (embedder.embed([vectorQuery]) as number[][])[0] ?? [] : [];
...
export async function recallHybridAsync(
	...
	const vectorQuery = prepareRecallQuery(query, embedder);
	const qv = vectorQuery.length > 0 ? (await embedder.embed([vectorQuery]))[0] ?? [] : [];
	...
	const preEmbedded: Embedder = {
		...
		...(embedder.queryPreprocessing !== undefined ? { queryPreprocessing: embedder.queryPreprocessing } : {}),
		embed: () => [qv],
	};
