...
export interface Embedder {
	...
	/**
	 * Bag-of-words embedders opt into content-only recall queries. Question
	 * scaffolding otherwise supplies spurious vector overlap even when BM25 finds
	 * no content term. Learned embedders omit this to retain the complete sentence.
	 * This affects query preparation only, never stored document embeddings.
	 */
	readonly queryPreprocessing?: "content-terms";
...
export class HashingEmbedder implements Embedder {
	readonly id: string;
	readonly queryPreprocessing = "content-terms" as const;
...
export class HrrEmbedder implements Embedder {
	...
	readonly dims: number;
	readonly queryPreprocessing = "content-terms" as const;
	// Keep the 0.3 recovery floor: raising it also suppresses genuine morphology
	// (e.g. "deploy" → "deploys"). Remove question scaffolding from recall queries
	// instead, so shared function words cannot supply the recovery signal. Random
	// feature collisions remain possible; this is not a learned semantic model.
