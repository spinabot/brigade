...
export async function reembedPending(
	...
	try {
		...
		opts.logger?.warn("reembed batch failed", { error: err instanceof Error ? err.message : String(err) });
	...
	if (updates.length > 0) opts.logger?.info("reembed pass", { embedded: updates.length });
