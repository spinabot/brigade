...
import { FactStore, type FactStoreOptions, type MemoryRecord, originBucketKey } from "../store/records.js";
 ...
 * markdown-vault re-render). Hosts may run it after turns or on an idle cadence.
 * Zero model cost; idempotent. Every internally opened store receives the same
 * per-instance host ports; standalone callers default to filesystem persistence.
 *
...
export function runMemoryMaintenance(
	...
	storeOptions: FactStoreOptions = {},
): void {
	try {
		runDecayGc(workspaceDir, undefined, storeOptions);
	} catch (err) {
	...
	try {
		...
		runCurator(new FactStore(workspaceDir, storeOptions), {
	...
	if (onContradictions) {
		try {
			const active = new FactStore(workspaceDir, storeOptions).list();
