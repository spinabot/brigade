 ...
 *   • Both BM25×eff and the SERVED hybrid abstain (0 violations on no-answer queries).
 *
 * The model-free recovery lane must not mistake shared question scaffolding for
 * evidence. Its content-only query preprocessing fixes the two original HRR
 * false positives without raising the cosine floor or sacrificing inflection
 * recovery (e.g. "deploy" → "deploys"). The hybrid unit tests separately cover
 * unseen negative queries, morphology, and learned-embedder synonym recovery.
 * Zero violations here is a fixture regression gate, not a guarantee that a
 * finite-dimensional model-free embedder never produces unrelated-text collisions.
 *
...
describe("hard gold — competition discriminates the recall lanes (the wins)", () => {
	it("Tideline's modulation + trust-weighted hybrid out-rank plain BM25/FTS", async () => {
		...
		// serves, so its abstention MUST be gated — not merely reported. Keep every
		// original no-answer case and require the recovery lane to stay quiet.
		assert.equal(hybrid.abstentionViolations, 0, "production hybrid must abstain on every hard-gold no-answer query");
	});
});
