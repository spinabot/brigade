...
describe("assembleBrigadeToolset — senderIsOwner gating", () => {
	it("default (no senderIsOwner) preserves backward-compatible owner behaviour", async () => {
		...
		assert.equal(ts.customTools.length, 21); // + transcribe_audio: analyze_media + make_document + edit_document + composio + connect_channel + message_action + find + generate_image + generate_speech + transcribe_audio + manage_provider + manage_access + manage_channel_access + manage_memory + oauth_authorize + recall + read_memory + write_memory + agents_list + manage_agent + manage_skill
	});
